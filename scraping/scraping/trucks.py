'''
This Python script is designed to scrape NASCAR race data from a website using Selenium and BeautifulSoup. It contains two main functions: race_results and trucks_racing.
...
def truck_racing(start, stop=None):
    ...
    for season in seasons:
        ...
        season_df.to_csv(
            os.path.join('data', 'truck-series', 'scraped', f'trucks-{season}.csv'),
            index=False
        )
    ...
    all_races_df.to_csv(
        os.path.join('data', 'truck-series', 'scraped', 'truck-series-full-import.csv'),
        index=False
    )
