...
def series_racing(series, season):
    ...
    season_df.to_csv(
        os.path.join('data', f'{series}-series', 'scraped', f'{series}-{season}.csv'),
        index=False
    )
...
def update_all_series(season):
    ...
    for series in series_list:
        ...
        # Define paths
        csv_path = f'data/{series}-series/scraped/{series}-series-full-import.csv'
        processing_script = f'processing_scripts/{series}_processing.py'
...
subprocess.run(['Rscript', 'processing_scripts/write_rda_files.R'], check=True)
