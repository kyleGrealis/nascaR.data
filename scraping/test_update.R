# Function to update each race series.
# Load the series data
# Find last race number last race number for the current year
# Scrape new results since last update
...
results <- cup_series |>
  ...
  summarize(race = max(race_number)) |>
  pull(race)

# add 1 to the last race number so that we get the newest results
max_race <- results + 1

###############################################################

base_url <- 'https://www.racing-reference.info/season-stats/{the_season}/W/'
...
# cookies <- httr::GET("https://www.racing-reference.info")  # Initial request to capture cookies
the_season_page <- read_html(
  httr::GET(
    httr::user_agent(user_agent),
    glue::glue(base_url),
    headers
    # set_cookies(httr::cookies(cookies))  # Pass the cookies to the request
  )
)

print(glue::glue('Season: {the_season}'))
...
# Create a race loop:
for (i in max_race:length(the_race_links)) {
# for (i in 1:10) {
  ...
  the_result_table <- the_race |>
    ...
    mutate(season = the_season, race_number = i, .before = 1) |>
...
new_cup_series <- bind_rows(cup_series, season_table)
