...
If you need to export NASCAR results to a local file (e.g. for use in spreadsheet applications or external scripts), you can use `export_series()` to write the data to CSV or Parquet format:
...
The race results data is hosted publicly on Cloudflare R2 and can be downloaded directly in CSV or Parquet format using standard command-line tools like `curl` or `wget`. No R installation is required.

See `vignette("ingesting-nascar-data")` (or the [online documentation](https://www.kylegrealis.com/nascaR.data/articles/ingesting-nascar-data.html)) for more detailed ingestion guides, direct download URLs, and critical warnings on correctly ingesting NASCAR car numbers to preserve leading zeros.
...
These files are updated every Monday morning at 5AM EST during the racing season (February through November).
...
| `export_series()` | Export race data to a local CSV or Parquet file |
