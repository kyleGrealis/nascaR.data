...
check_integrity <- function(data, series_name) {
  ...
  # Check for duplicate rows (same Season + Race + Driver)
  dup_check <- data |>
  ...
  if (nrow(dup_check) > 0) {
    ...
    stop(
      paste0(
        'Found ', nrow(dup_check), ' duplicate rows in ', series_name,
        ' series. Sample duplicates:\n',
  ...
  # Check start positions are positive integers (excluding NA)
  invalid_start <- data |>
    dplyr::filter(!is.na(Start) & (Start < 1 | Start != as.integer(Start))) |>
    nrow()
  ...
  if (invalid_start > 0) {
    stop(
      paste0(
        ...
        series_name, ' series. Start must be positive integers or NA.'
      ),
      call. = FALSE
    )
  }
    ...
    suggestions <- purrr::map_chr(
      ...
      function(x) {
        if (!is.na(x$suggestion)) {
          paste0(x$unmatched, ' -> ', x$suggestion, ' (dist: ',
                 round(x$distance, 3), ')')
        } else {
