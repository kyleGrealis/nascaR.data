...
series: Cup (1949-present), NXS (1982-present), and Trucks
(1995-present). Explore driver, team, and manufacturer performance in a
race-by-race, season, or career format. This data has been expertly
curated and scraped with permission from
[DriverAverages.com](https://www.driveraverages.com).

> **Note for v3.0.0:** The second-tier series identifier has changed
> from `"xfinity"` to `"nxs"`. See
> [`vignette("migrating-to-nxs")`](https://www.kylegrealis.com/nascaR.data/articles/migrating-to-nxs.md)
> for details.
...
pak::pak("kyleGrealis/nascaR.data")
```

**Note:** This package requires the `arrow` package for reading parquet
data from cloud storage. It will be installed automatically.

## Loading Data

All data is served from cloud storage. Use
...
If you need to export NASCAR results to a local file (e.g. for use in
spreadsheet applications or external scripts), you can use
[`export_series()`](https://www.kylegrealis.com/nascaR.data/reference/export_series.md)
to write the data to CSV or Parquet format:

``` r

library(nascaR.data)

# Export Cup Series data to CSV (automatically guesses format from extension)
export_series("cup", "cup_series.csv")

# Export NXS data to Parquet
export_series("nxs", "nxs_series.parquet")
```

------------------------------------------------------------------------

## Command-Line & Direct Data Access (Non-R Users)

The race results data is hosted publicly on Cloudflare R2 and can be
downloaded directly in CSV or Parquet format using standard command-line
tools like `curl` or `wget`. No R installation is required.

See
[`vignette("ingesting-nascar-data")`](https://www.kylegrealis.com/nascaR.data/articles/ingesting-nascar-data.md)
(or the [online
documentation](https://www.kylegrealis.com/nascaR.data/articles/ingesting-nascar-data.html))
for more detailed ingestion guides, direct download URLs, and critical
warnings on correctly ingesting NASCAR car numbers to preserve leading
zeros.
...
# NXS Series (1982-present)
curl -O https://nascar.kylegrealis.com/nxs_series.csv

# Truck Series (1995-present)
curl -O https://nascar.kylegrealis.com/truck_series.csv
```
...
``` bash
# Cup Series
curl -O https://nascar.kylegrealis.com/cup_series.parquet

# NXS Series
curl -O https://nascar.kylegrealis.com/nxs_series.parquet

# Truck Series
curl -O https://nascar.kylegrealis.com/truck_series.parquet
```

These files are updated every Monday morning at 5AM EST during the
racing season (February through November).
...
competes against Formula-1 and IndyCar for the top viewership spot.
Approximately 3.22 million people watch a race on any given weekend
throughout the season. The nascaR.data package is the result of wanting
to share a passion for the sport and provide an option to the typical
go-to packages when learning new data cleaning & visualization tools.

## Data Structure

Three series are available via
[`load_series()`](https://www.kylegrealis.com/nascaR.data/reference/load_series.md):

- `load_series("cup")`: NASCAR Cup Series race results (1949-present)
- `load_series("nxs")`: NASCAR NXS (second-tier) race results
  (1982-present)
- `load_series("truck")`: NASCAR Craftsman Truck Series results
  (1995-present)
...
Data is sourced with permission from DriverAverages.com and is
automatically updated every Monday at 5AM EST during the racing season
(February through November).
...
Use the suite of `get_*_info()` functions to examine specific
...
| [`export_series()`](https://www.kylegrealis.com/nascaR.data/reference/export_series.md) | Export race data to a local CSV or Parquet file |
...
We’re open to suggestions! If you have ideas for new features, please
