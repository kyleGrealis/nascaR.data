...
**What changed:** v3.0.0 introduces a two-tier caching system (memory +
disk).

**How it works:**

1.  **First call:** Downloads data from R2 and caches in memory and on
    disk
2.  **Subsequent calls in same session:** Instant retrieval from memory
    cache
3.  **Subsequent calls in new sessions:** Fast retrieval from disk cache

**Disk cache location:** Uses CRAN-approved
[`tools::R_user_dir()`](https://rdrr.io/r/tools/userdir.html) path:
...
**Why:** Caching dramatically improves performance. After the first
download, data loads instantly without network requests.
