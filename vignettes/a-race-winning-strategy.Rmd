...
`nascaR.data` provides access to 3 datasets with race-by-race results for each NASCAR series, plus 6 helper functions to make exploring the data easier. Let's check our gauges and see what's under the hood:
...
- `cup_series`: NASCAR Cup Series races from 1949-present  
- `xfinity_series`: NASCAR Xfinity Series races from 1982-present  
- `truck_series`: NASCAR Truck Series races from 1995-present  

Each dataset contains race results with 19 columns including driver, team, manufacturer, finishing position, laps led, and more. Use `?cup_series` to view a complete list of variable descriptions.

The package also provides helper functions to quickly find and analyze drivers, teams, and manufacturers:  
- `find_driver()`, `find_team()`, `find_manufacturer()`: Search for exact names  
...
cup_series |>
  ...
  geom_line(color = '#fb9f00', size = 1.2) +
...
The trend shows consistent improvement, with better average finishes in recent seasons. Lower numbers are better — a reversed y-axis makes this clearer.
...
### Finding drivers and getting their statistics

```{r}
#| echo: true
#| eval: false
# Find a driver (handles partial matching)
find_driver('bell')
```

The `find_driver()` function returns exact matches if available, or suggests close matches if your search term is ambiguous.

```{r}
#| echo: true
#| eval: false
# Get comprehensive driver statistics
get_driver_info('Christopher Bell', series = 'cup', type = 'summary')
```
...
#| echo: true
#| eval: false
# Find team (handles partial names)
find_team('gibbs')

# Get team statistics
get_team_info('Joe Gibbs Racing', series = 'cup', type = 'summary')
...
This vignette gives you a foundation for exploring NASCAR data with the `nascaR.data` package. The three datasets (`cup_series`, `xfinity_series`, `truck_series`) provide comprehensive race results, while the helper functions (`find_driver()`, `get_driver_info()`, and their team/manufacturer equivalents) make data exploration straightforward.
