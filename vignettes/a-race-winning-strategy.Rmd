...
- `load_series("xfinity")`: NASCAR Xfinity Series races from 1982-present
- `load_series("truck")`: NASCAR Truck Series races from 1995-present

Each dataset contains race results with 19 columns including driver, team, manufacturer, finishing position, laps led, and more. Use `?load_series` to view a complete list of variable descriptions.
...
This vignette gives you a foundation for exploring NASCAR data with the `nascaR.data` package. Use `load_series()` to access comprehensive race results for all three series, while the helper functions (`find_driver()`, `get_driver_info()`, and their team/manufacturer equivalents) make data exploration straightforward.
